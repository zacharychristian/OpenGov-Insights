...
def get_financial_data():
    ...
    for year in fiscal_year:
        for agent in range(len(agency_id)):
            ...
            if resp and len(resp) > 0:
                json = resp[0]
                budget_authority_amount.append(float(json.get('budget_authority_amount', 0)))
                obligated_amount.append(float(json.get('obligated_amount', 0)))
                outlay_amount.append(float(json.get('outlay_amount', 0)))
            else: 
                budget_authority_amount.append(0)
                obligated_amount.append(0)
                outlay_amount.append(0)

    logger.info("API calls for get_financial_data completed.")
    ...
    logger.info("get_financial_data completed.")
    return df