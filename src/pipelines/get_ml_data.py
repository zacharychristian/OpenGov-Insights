...
def get_ml_data(fiscal_year = 2024, quarter = 1, max_retries=5, backoff_factor=2):
    import time
    import requests
...
def transform_ml_data(last_fiscal_year = 2025):
    """
    ...
    for year in range(2015, last_fiscal_year+1): 
        for quarter in range(1,5): #Only 4 quarters in a year. 1,2,3,4
            resp = get_ml_data(fiscal_year = year, quarter = quarter)         
    ...
    logger.info("API calls for transform_ml_data completed.")
    ...
    logger.info("transform_ml_data completed.")
    return df